...
$pageArrey = explode('nt/search', $_SERVER['REQUEST_URI']);
...
foreach( $data as $da ){
	...
	foreach( $nfArray  as $va ){
		if(is_numeric($da[$modelName][$va])){
			$da[$modelName][$va] = number_format($da[$modelName][$va]).'円';
		}
	}
	for($a = 1; $a <= 3; $a++) {
...
<div class="post">
	<table width="693">
		...
		<td align="center" width="139">
			<span class="pu">'.$da[$modelName]['yatin_k'].'</span>円<br />
			'.$da[$modelName]['kyoueki_k'].'<br />
			'.$hosyou_kuArr[$da[$modelName]['hosyou_ku']].' '.$da[$modelName]['hosyou_k'].'<br>
			'.$kaiyaku_kuArr[$da[$modelName]['kaiyaku_ku']].' '.$da[$modelName]['kaiyaku_k'].'
		</td>
		<td align="center" width="74">
			<span class="pu">'.$madori1Arr[$da[$modelName]['madori1']].
			$madori2Arr[$da[$modelName]['madori2']].'</span><br />'.$da[$modelName]['heibei'].'㎡<br />
			'.$da[$modelName]['syozaikai'].'階部分
		</td>
		<td align="center" width="181">
			'.$da[$modelName]['bu_zyuusyo1'].'<br />
		...
		<td align="center" width="75">
			<a href="'.$this->webroot.'AdminRent/daityou?id='.$da[$modelName]['id'].'" target="_blank">台帳</a><br>
			<a href="'.$this->webroot.'AdminRent/kanban?id='.$da[$modelName]['id'].'" target="_blank">看板</a><br>
			<a href="'.$this->webroot.'AdminRent/zyouhou?id='.$da[$modelName]['id'].'" target="_blank">情報</a>
		</td>
		</tr>
		<tr>
...
<div id="container">
	...
	<div id="header">
		<!-- ナビゲーションバー -->
		<div id="nav">
		<p id="page_title">Property Search System 管理 <font color="#FF0000">賃貸</font> 検索</p>
		</div>
	</div>
...
<li id="search">種別&nbsp;
<?php echo $this->Form->select('sy',$syubetuArr2,array('empty'=>false))."\n"; ?>
</li>
<li id="search">沿線&nbsp;
...
<br /><font size="-2">　（物件名・家主名・管理会社・<br />　　コメント・備考）</font>
</li>
<li id="search">
お客様HP表示なし&nbsp;
...
<?php echo $this->Form->end(array('label'=>'検索','div'=>false)); ?>
			...
			<ul>
				...
				<ul>
					<li id="submit">
					<div class="submit"><br />
					<form method="post" action="<?php echo $this->webroot; ?>AdminRent/add">
...
<?php echo $this->Paginator->sort('yatin_k','賃料'); ?><br />
共益費<br />敷金 保証金<br />礼金 解約引</td>
<td width="74" align="center" class="bkco">
<?php echo $this->Paginator->sort('madori1','間取り'); ?><br />
<?php echo $this->Paginator->sort('heibei','㎡'); ?><br />
階数</td>
<td width="181" align="center" class="bkco">住所<br />最寄沿線 駅 徒歩<br />物件名</td>
...
?>
	...
	<div id="footer">
		<div id="footer-inner">
			<div class="copyright">
				<hr width="950" size="1" />
				Property Search Copyright(C) <a href="https//realestateguide.jp" target="_blank">ITS</a>
			</div>
		</div>
	</div>
</div>
