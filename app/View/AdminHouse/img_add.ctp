...
foreach( $imgArray as $va ){
	if(!empty($this->request->data[$modelName][$va])){
		if($this->request->data[$modelName][$va] == 1){
			$imgAr[$va] = '<img src="../img/house/gazou/'.$this->request->data[$modelName]['id'].'_'.$i_ind.
			'.jpg?'.time().'" width="200" border="0" />';
			$i_ind++;
		}else{
			$imgAr[$va] = '<img src="../img/house/gazou/'.$this->request->data[$modelName]['id'].'_'.$i_ind.
			'.jpg?'.time().'" height="200" border="0" />';
			$i_ind++;
		}
	}else{
		$imgAr[$va] = '';
...
<p id="page_title">不動産検索システム ebs3 管理 <font color="#0000FF">売買</font> 画像</p>
<div id="modoru">
