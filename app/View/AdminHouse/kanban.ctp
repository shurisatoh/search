...
if(isset($this->request->data['fcolor'])){
	$filepointer=fopen('../Vendor/configHouseKanban.php', "a+");
	...
	App::import('Vendor', 'configHouseKanban');
...
if(!empty($data[$modelName]['madori1']) && !empty($data[$modelName]['madori2'])){
	$zyouken .= '
	<li><p>間取り：</p>'.$madori1Arr[$data[$modelName]['madori1']].$madori2Arr[$data[$modelName]['madori2']].'</li>';
}else{
	$zyouken .= '
		<li>&nbsp;</li>';
}
if(!empty($data[$modelName]['kouzou'])){
	$zyouken .= '
	<li><p>構　造：</p>'.$kouzouArr[$data[$modelName]['kouzou']].'</li>';
}else{
	$zyouken .= '
		<li>&nbsp;</li>';
}
if(!empty($data[$modelName]['tiku_nen'])){
	$zyouken .= '
	<li><p>築年月：</p>'.$data[$modelName]['tiku_nen'].'年'.$data[$modelName]['tiku_tuki'].'月築</li>';
}else{
	$zyouken .= '
		<li>&nbsp;</li>';
}
...
<div class="leftcol">
<p  id="page_title">不動産検索システム ebs3 管理 <font color="#0000FF">売買</font> 看板</p>
...
<table width="1260" height="880" border="0" align="center" cellpadding="0" cellspacing="0" id="rent">
  <tbody>
    <tr>
      ...
      <span class="rent_mozityuu">価格：</span><?php echo $data[$modelName]['kakaku']; ?><span class="rent_mozityuu">万円</span>
      </td>
    </tr>
    <tr>
